...
class DetallePedidoService:
    ...
    def get_by_pedido(self, pedido_id: int) -> List[DetallePedidoPublic]:
        with DetallePedidoUnitOfWork(self._session) as uow:
            items = uow.detalles.get_by_pedido(pedido_id)
            result = [DetallePedidoPublic.model_validate(i) for i in items]
        return result

    def get_item(self, pedido_id: int, producto_id: int) -> DetallePedidoPublic:
        with DetallePedidoUnitOfWork(self._session) as uow:
            ...
            result = DetallePedidoPublic.model_validate(item)
        return result
